package mv.mossuh.mopets.MODEL.Player;
...
import mv.mossuh.mopets.UTILITIES.Pet.PetsGetter;
...
public class PetsPlayer {
    ...
    public void setPets(List<Pet> pets) {
        if (Config.MULTIPLE_PETS) {
            List<Pet> filteredPets = UtilMethods.cleanConflicts(pets);

            List<Pet> oldPets = new ArrayList<>(this.pets);
            this.pets = filteredPets;

            // DEACTIVATED
            for (Pet pet : oldPets) {
                if (pet.isPet() && !this.pets.contains(pet)) {
            ...
            for (Pet pet : this.pets) {
                if (pet.isPet()) {
                    ...
                    for (Pet old : oldPets) {
                        if (old.getPetUUID().equals(pet.getPetUUID())) {
                            wasActive = true;
                            break;
                        }
                    }
                    if (!wasActive) {
                        Bukkit.getPluginManager().callEvent(new PlayerChangePetEvent(uuid, pet, ChangePetType.ACTIVATED));
                    }
                }
            }
            return;
            ...
            for (Pet pet : oldPets) {
                if (pet.isPet()) {
                    Bukkit.getPluginManager().callEvent(
                            new PlayerChangePetEvent(uuid, pet, ChangePetType.DEACTIVATED));
                }
            }
            return;
        }
        ...
        // DEACTIVATED: pet anterior
        if (!this.pets.isEmpty()) {
            Pet oldPet = this.pets.get(0);
            if (oldPet.isPet()) {
                Bukkit.getPluginManager().callEvent(
                        new PlayerChangePetEvent(uuid, oldPet, ChangePetType.DEACTIVATED));
            }
        }
        ...
        // ACTIVATED: nueva pet
        Bukkit.getPluginManager().callEvent(
                new PlayerChangePetEvent(uuid, newPet, ChangePetType.ACTIVATED));
    }
